...
class demo:
    def run(self):
        ...
        self._smpl_dataset_path = './data/Oct_22_stevens/output_pairs/'
        ...
        self._realsense_K = np.array([[1361.8736572265625, 0, 963.2017211914062],[0, 1361.03076171875, 552.251220703125],[0, 0, 1]], dtype=np.float32)
        ...
        cfg_smpl_filter_bb = smplpact.renderer_create_settings_smpl_filter_bounding_box([670, 0, 1400, 1080], [smplpact.smpl_joints_openpose.MidHip, smplpact.smpl_joints_openpose.LHip, smplpact.smpl_joints_openpose.RHip], [0.4, 0.3, 0.3], 0.59)
        cfg_smpl_filter_ff = smplpact.renderer_create_settings_smpl_filter_forward_face()
        cfg_smpl_filter_ef = smplpact.renderer_create_settings_smpl_filter_exponential_single([0.25, 0.25, 0.25, 0.9])
        cfg_smpl_filter_ow = smplpact.renderer_create_settings_smpl_filter_fixed_joints()

        self._offscreen_renderer = smplpact.renderer_context(cfg_offscreen, cfg_scene, cfg_camera, cfg_camera_transform, cfg_lamp, cfg_smpl_model, None, cfg_smpl_filter_bb, cfg_smpl_filter_ff, cfg_smpl_filter_ef, None, None, cfg_smpl_filter_ow)
        ...
        # Load dataset
        files, folders = smplpact.path_scan(self._smpl_dataset_path, folders_sort=True, folders_key=lambda x : int(x.split('_')[-1]))
        index = 0
        ...
        while (True):
            # Get paths
            self._smpl_message_path = os.path.join(folders[index], 'pose.json')
            self._probe_message_path = os.path.join(folders[index], 'aruco.json')
            self._realsense_image_path = os.path.join(folders[index], 'realsense_image.jpg')
    ...
    def _paint(self):
        ...
        smpl_data = smpl_meshes.get('patient', None) #smpl_result.at(0)
