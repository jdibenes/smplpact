...
class demo:
    ...
    def _loop(self, camerahmr_message):
        ...
        if (self._color_task is None):
            self._color_task = self._offscreen_renderer.smpl_paint_color_solid(smpl_mesh_id, self._face_segmentation['rightUpLeg'], self._brush_color_center, 1, timeout=0.5, fixed=True, manual=True, color_id=-3, task_id=-3)
