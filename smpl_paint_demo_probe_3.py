...
class demo:
    def run(self):
        ...
        self._smpl_dataset_path = './data/calibration_captures'
        ...
        while (True):
            # Get paths
            self._realsense_color_path = files[(4*index) + 0]
            self._realsense_depth_path = files[(4*index) + 1]
            self._probe_message_path = files[(4*index) + 2]
            self._smpl_message_path = files[(4*index) + 3]
            
            index = (index + 1) % (len(files) // 4)

            # Load image
            self._realsense_color = cv2.imread(self._realsense_color_path, cv2.IMREAD_COLOR_BGR)
            if (self._realsense_color is None):
                continue

            # Load depth
            self._realsense_depth = np.load(self._realsense_depth_path)
            if (self._realsense_depth is None):
                continue
    ...
    def _paint(self):
        ...
        # Set probe position
        probe = self._probe_message['probe']
        ...
        cv2.circle(self._realsense_color, center, self._probe_image_radius, self._probe_image_color, self._probe_image_thickness)

        # Show image
        cv2.imshow('RealSense', self._realsense_color)
